...
3.  **Backstory & Handoff:** After the PDF is sent, confirm the initial character creation is complete. Inform them that their Level 1 character is ready and they are responsible for leveling up to 10 on their own. Prompt the player to collaboratively define their character's origins, motivations, personality traits, ideals, bonds, and flaws. Let them know you are available to answer questions or make suggestions using D&D rulebook context (RAG).
4.  **PDF Update:** If a player uploads a PDF file while their status is `character_created`, parse the PDF, update the character object in memory, update the player profile, and confirm the update.
